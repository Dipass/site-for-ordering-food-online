...
const Addfood = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        const foodData = { name, price, foodImage, catagory, location, description, weight };

        const res = await axios.post("http://localhost:8000/api/v1/food/addfood", { name, price, foodImage, catagory, location, description, weight }, {
            headers: {
    ...
    return (
        <div className="addfood">
            <div className="w-full mx-auto pt-[16vh]  bg-green-100/10 mb-20">
                <form className=' ease-in duration-300 w-[80%] sm:w-max shadow-sm backdrop-blur-md bg-white/80 lg:w-max mx-auto rounded-md px-8 py-5' onSubmit={handleSubmit}>
                    <NavLink to="/">
                        <img src={logo2} alt="" className="logo mx-auto mb-6 cursor-pointer text-center" />
                    </NavLink>
                    ...
                    <button className=" bg-[#f54748] active:scale-90 transition duration-150 transform hover:shadow-xl shadow-md w-full rounded-full px-8 py-2 text-xl font-medium text-white mx-auto text-center mb-23 mt-5 " type='submit'>Add food</button>
